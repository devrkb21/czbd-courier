...
readonly class PaperflyService implements CourierServiceInterface
{
    ...
    public function getToken(array $account): string
    {
        ...
        return Cache::remember($cacheKey, 3300, function () use ($account) {
            $response = $this->httpClient()->post(self::BASE_URL . "/authentication/login_using_password.php", [
        ...
        try {
            ...
            foreach ($this->accounts as $account) {
                try {
                    ...
                    // so that case is treated as a failed account (and the
                    // pool moves on) instead of a false "0 success / 0 cancel".
                    if (!is_array($data) || !array_key_exists('totalRecords', $data)) {
                        $lastError = ['error' => 'Unexpected response from Paperfly (possibly rate limited)', 'status' => $response->status()];
                        continue;
                    }
